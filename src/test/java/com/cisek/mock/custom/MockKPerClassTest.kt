...
class MockKPerClassTest {
    ...
    private var robot = CustomViewRobot(presenter)

    @BeforeEach
    fun setUpEach() {
        presenter.unbindIntents()
        unmockkObject(navigator, interactor)
        every { interactor.partialStateObserver() } returns dataSubject
